...
def check_user_password_is_correct(db, username, password):
    ...
    if not verify(password, user_obj.hashed_password):
        return False
    return user_obj
...
def toggle_upvote(article_id, user_id_value, db):
    existing_upvote = db.execute(
        select(user_news_association_table).where(
            user_news_association_table.c.news_articles_id == article_id,
            user_news_association_table.c.user_id == user_id_value,
        )
    ).scalar()
    ...
    if existing_upvote:
        ...
        insert_stmt = insert(user_news_association_table).values(
            news_articles_id=article_id, user_id=user_id_value
        )
